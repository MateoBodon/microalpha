...
- Root cause confirmed: pre-fix WRDS debug candidates generated nonzero target weights (`targets_nonzero=61`, `sum_abs_weights=3.56`) but `orders_created=0` with drop reasons dominated by `max_exposure` (40) and `max_single_name_weight` (21), so broker accepts/fills remained zero (`orders_accepted=0`, `fills=0`, `diagnostic_reason=no_orders`). Post-fix diagnostics show cap clipping with orders accepted and filled (`orders_created=12`, `orders_accepted=12`, `fills=12`, `filled_notional=2,964,947.94`).
- WRDS debug runs:
