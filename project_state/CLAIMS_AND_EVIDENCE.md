...
source_event: preregistered first-filed SEC vintage validation negative
...
| Historical SEC accounting values can be reconstructed without current-value backfill for the tested concepts. | supported for the exact extractor contract | `docs/strategy/MICROALPHA_SEC_VINTAGE_20260711.yaml`; `src/microalpha/research/crsp_v2_sec_vintage.py`; `tests/test_crsp_v2_sec_vintage.py`; SEC-vintage integrity report | Support is limited to accession-matched `Assets`, `NetIncomeLoss`, and operating-cash-flow facts in original XBRL 10-Ks. Bulk members are unpartitioned and later bytes are decoded but never selected. |
| The preregistered first-filed SEC cash-earnings acceleration mechanism is a viable replacement. | rejected by complete predeclared gate | `docs/strategy/MICROALPHA_SEC_VINTAGE_20260711.yaml`; `docs/artifacts/resume/wrds/2026-07-11T19-34-00Z-sec-vintage/`; local result manifest SHA-256 `1ea92a323a254422772a8edfd86d0fd8d70b806a296190acaec1fb816379a3be` | HAC Sharpe `0.4736` and CAGR `1.73%` improved materially on frozen momentum, but Sharpe missed the absolute `0.50` gate and the 600 bps borrow/2x-cost stress had Sharpe `-0.1034` and CAGR `-0.45%`. Archive without retuning. |
| The preregistered SEC reporting-timeliness mechanism is empirically viable. | unassessed; execution disabled | `docs/strategy/MICROALPHA_SEC_REPORTING_TIMELINESS_20260711.yaml`; `src/microalpha/research/crsp_v2_sec_timing.py`; `tests/test_crsp_v2_sec_timing.py` | Source feasibility and chronology are tested, but no validation return column has been projected and no coverage or performance claim exists. |
| The 2023-2025 final holdout remained sealed throughout the recorded CRSP-v2 and accounting campaigns. | supported by bound access receipts and result manifests | panel manifest SHA-256 `0e625c05853bfed6e8b811bbd3df86d0fe64a05cf9929ede537299adc851710d`; mechanism result manifests; QVPI and SEC-vintage integrity reports | This proves the recorded runners did not use holdout outcomes; it does not establish future behavior outside these exact artifacts. |
