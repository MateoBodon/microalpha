...
| Selection cannot read sealed holdout outcome rows through the new panel adapter. | supported for code contract and bounded integration | `src/microalpha/research/crsp_v2_panel.py`; `tests/test_crsp_v2_panel.py` | Must be rechecked on the eventual derived manifest before model selection. |
...
| Industry-neutral universe, turnover, capacity, and cost accounting primitives are implemented. | supported for bounded tests | `tests/test_crsp_v2_research.py` | No empirical performance claim follows from mechanism tests. |
