...
class SimulatedBroker:
    ...
    def on_market_tick(self, market_event, events_queue):
        ...
        for order in self._open_orders:
            if order.timestamp == market_event.timestamp:
                print(f"      BROKER (TWAP): Executing child order for {order.quantity} {order.symbol}.")
    ...
    def _schedule_twap_orders(self, order_event):
        ...
        if child_quantity == 0:
             ...
             single_child = OrderEvent(future_timestamps[0], order_event.symbol, total_quantity, order_event.direction)
             self._open_orders.append(single_child)
             return

        print(f"      BROKER (TWAP): Scheduling {total_quantity} shares over {effective_num_ticks} ticks.")
